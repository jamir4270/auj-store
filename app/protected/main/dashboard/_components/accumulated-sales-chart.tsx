...
export function AccumulatedSalesChart() {
  ...
  const processData = (orders: Order[]): ChartDataPoint[] => {
    const groupedData: Record<
      string,
      { date: string; originalDate: Date; dailySales: number }
    > = {};
    ...
    orders.forEach((order) => {
      ...
      groupedData[dateKey].dailySales += Number(order.total || 0);
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("Error fetching accumulated sales:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row justify-between">
        <div>
          <CardTitle>Accumulated Sales</CardTitle>
          <CardDescription>
      ...
      <CardFooter className="flex-col items-start gap-2 text-sm">
        <div className="flex gap-2 leading-none font-medium">
          Total Sales: {totalAccumulated.toLocaleString()}{" "}
